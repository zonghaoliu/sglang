...
class ExpertRecorderBuffer:
    def __init__(
        ...
        self._buffer = torch.empty(
            buffer_size_mb * 1024 * 1024 // dtype.itemsize,
            dtype=dtype,
            device=device,
            pin_memory=True,  # for non-blocking device-to-host copy
        ).fill_(-1)
        self._write_ptr = 0
        self._is_full = False

    def _backup_tensor_async(self, tensor: torch.Tensor):
        with torch.cuda.stream(self._log_stream):
            ...
            if end <= self._buffer.numel():
                self._buffer[start:end].copy_(tensor, non_blocking=True)
                self._write_ptr = end
            else:
                # notice the user
                logger.info(
    ...
    def store(self, tensor: torch.Tensor) -> GathererTensorAddress:
        ...
        return self._backup_tensor_async(t)

    def get(self, address: GathererTensorAddress):
        flat_slice = self._buffer[address.offset : address.offset + address.length]

        # Reshape to original dimensions
        return flat_slice.reshape(address.shape).clone()
...
class _ExpertDistributionRecorderReal(ExpertDistributionRecorder):
    def __init__(
        ...
        self._buffer = ExpertRecorderBuffer(
